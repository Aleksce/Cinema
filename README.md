...
## Ошибка CS0246: `HttpClient` не найден

Если Visual Studio показывает `CS0246` в `TmdbClient.cs` для `HttpClient`, проверьте:

1. В начале файла есть `using System.Net.Http;`.
2. Проект на `net8.0-windows` и включены `ImplicitUsings`.
3. Сделайте `Clean` + `Rebuild Solution` после правки.

В актуальной версии проекта директива `using System.Net.Http;` уже добавлена.


## Почему раньше TMDB "не менялся"

Раньше фильмы загружались из TMDB только при первом запуске (когда таблица сеансов пустая).
Если `cinema.db` уже существовал, старые данные оставались.

Теперь добавлена кнопка **«Синхр. TMDB»** в шапке приложения: она принудительно перезагружает список фильмов из TMDB и обновляет сеансы/места в БД.
